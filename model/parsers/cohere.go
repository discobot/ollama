...
func (p *CohereParser) eat(done bool) (content string, thinking string, calls []api.ToolCall, more bool) {
	...
	case cohereAwaitingBlock:
		// Between blocks: look for the next START_TEXT or START_ACTION,
		// skipping whitespace and the end-of-turn token.
		if idx := strings.Index(buf, cohereStartText); idx != -1 {
			p.resetBuffer(buf[idx+len(cohereStartText):])
			p.state = cohereCollectingContent
			return "", "", nil, true
		}
		...
		// Tolerate bare content with no START_TEXT wrapper: emit anything
		// that cannot be the start of a tag once enough context arrives.
		...
		if !strings.HasPrefix(trimmed, "<|") || (done && !strings.Contains(trimmed, cohereStartText) && !strings.Contains(trimmed, cohereStartAction)) {
			if maybePartialTag(trimmed) && !done {
				return "", "", nil, false
			}
			p.buffer.Reset()
			p.state = cohereCollectingContent
	...
	case cohereCollectingContent:
		if idx := strings.Index(buf, cohereEndText); idx != -1 {
			content = buf[:idx]
			p.resetBuffer(buf[idx+len(cohereEndText):])
			p.state = cohereAwaitingBlock
			return content, "", nil, true
		}
		// Strip a trailing end-of-turn marker if the model skipped END_TEXT.
		if idx := strings.Index(buf, cohereEndOfTurn); idx != -1 {
			content = buf[:idx]
			p.resetBuffer(buf[idx+len(cohereEndOfTurn):])
			p.state = cohereAwaitingBlock
			return content, "", nil, true
		}
		keep := max(overlap(buf, cohereEndText), overlap(buf, cohereEndOfTurn))
		emitEnd := len(buf) - keep
		if emitEnd > 0 {
			content = buf[:emitEnd]
			p.resetBuffer(buf[emitEnd:])
...
// maybePartialTag reports whether s could be a prefix of one of the parser's
// tags (so we should wait for more output before treating it as content).
func maybePartialTag(s string) bool {
	for _, tag := range []string{cohereStartText, cohereStartAction, cohereEndOfTurn} {
